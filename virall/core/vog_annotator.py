...
class VOGAnnotator:
    ...
    def _parse_blast_results(self, blast_xml: Path) -> Dict[str, Dict]:
                    ...
                    for alignment in record.alignments:
                        for hsp in alignment.hsps:
                            if hsp.expect < best_evalue:
                                ...
                                best_hit = {
                                    'vog_id': alignment.title.split()[0],
    ...
    def _add_functional_annotations(self, blast_annotations: Dict[str, Dict]) -> Dict[str, Dict]:
        """Add functional annotations from VOG database."""
        if not self.vog_annotations or not self.vog_annotations.exists():
            logger.warning("VOG annotations file not found")
            return blast_annotations
        
        try:
            # Load VOG annotations
            vog_df = pd.read_csv(self.vog_annotations, sep='\t')
            vog_dict = vog_df.set_index('vog_id').to_dict('index')
            
            # Add functional information to each hit
            for query_id, hit in blast_annotations.items():
                vog_id = hit['vog_id']
                if vog_id in self.vog_annotations:
                    vog_info = self.vog_annotations[vog_id]
                    hit.update({
                        'vog_name': vog_info.get('name', 'Unknown'),
                        'vog_function': vog_info.get('function', 'Unknown'),
                        'vog_category': vog_info.get('category', 'Unknown'),
                        'vog_consensus_function': vog_info.get('consensus_function', 'Unknown'),
                        'vog_consensus_annotation': vog_info.get('consensus_annotation', 'Unknown')
                    })
                    logger.debug(f"VOG {vog_id}: {vog_info.get('function', 'Unknown')}")
                else:
                    logger.debug(f"VOG {vog_id} not found in annotations")
                    hit.update({
                        'vog_name': 'Unknown',
                        'vog_function': 'Unknown',
                        'vog_category': 'Unknown',
                        'vog_consensus_function': 'Unknown',
                        'vog_consensus_annotation': 'Unknown'
                    })
            
        except Exception as e:
            logger.error(f"Error adding functional annotations: {e}")
