...
class AssemblyValidator:
    ...
    def run_quast(
        ...
        """
        if output_dir is None:
            output_dir = self.temp_dir / "quast_output"
        
        output_dir = Path(output_dir)
        output_dir.mkdir(parents=True, exist_ok=True)
        
        logger.info(f"Running QUAST on {genome_file}")
        
        cmd = [
            "conda", "run", "-n", "quast_env", "quast",
            genome_file,
            "-o", str(output_dir),
            "--threads", str(self.threads),
            "--min-contig", "100",  # Lower minimum contig size for small viral contigs
            "--no-snps",  # Disable SNP detection for small contigs
            "--no-gc"  # Disable GC content analysis for small contigs
        ]
        ...
        result = subprocess.run(cmd, capture_output=True, text=True)
        if result.returncode != 0:
            logger.warning(f"QUAST failed: {result.stderr}")
            # For small viral contigs or any QUAST failure, return basic statistics instead of failing completely
            logger.info("QUAST failed, returning basic statistics as fallback")
            return {
                "status": "completed",
                "error": f"QUAST failed: {result.stderr}",
                "n50": 0,
                "total_length": 0,
                "num_contigs": 0,
                "largest_contig": 0
        ...
        # Parse QUAST results
        results = self._parse_quast_results(output_dir)
        logger.info("QUAST completed successfully")
        
        return results
    
    def run_comprehensive_quast(
        self, 
        assembly_dir: Path,
        output_dir: Optional[str] = None
    ) -> Dict[str, Union[str, float, int]]:
        """
        Run comprehensive QUAST evaluation on multiple assembly files.
        
        Args:
            assembly_dir: Path to SPAdes assembly directory
            output_dir: Output directory (optional)
            
        Returns:
            Dictionary containing comprehensive QUAST results
        """
        if output_dir is None:
            output_dir = self.temp_dir / "comprehensive_quast"
        
        output_dir = Path(output_dir)
        output_dir.mkdir(parents=True, exist_ok=True)
        
        # Main assemblies to compare
        assembly_files = []
        labels = []
        
        # Check for main assembly files
        main_files = {
            "contigs": assembly_dir / "contigs.fasta",
            "scaffolds": assembly_dir / "scaffolds.fasta",
            "before_rr": assembly_dir / "before_rr.fasta"
        }
        
        for label, file_path in main_files.items():
            if file_path.exists():
                assembly_files.append(str(file_path))
                labels.append(label)
        
        if not assembly_files:
            logger.warning("No assembly files found for comprehensive QUAST")
            return {"status": "failed", "error": "No assembly files found"}
        
        logger.info(f"Running comprehensive QUAST on {len(assembly_files)} assembly files")
        ...
        result = subprocess.run(cmd, capture_output=True, text=True)
        if result.returncode != 0:
            logger.warning(f"Comprehensive QUAST failed: {result.stderr}")
            return {"status": "failed", "error": result.stderr}
        
        # Parse comprehensive QUAST results
        results = self._parse_quast_results(output_dir)
        results["status"] = "completed"
        results["evaluation_type"] = "comprehensive"
        results["assembly_files"] = assembly_files
        results["labels"] = labels
        
        logger.info("Comprehensive QUAST completed successfully")
        return results
    
    def run_viral_quast(
        self, 
        viral_files: List[str],
        output_dir: Optional[str] = None
    ) -> Dict[str, Union[str, float, int]]:
        """
        Run QUAST specifically on viral contig files with viral-specific metrics.
        
        Args:
            viral_files: List of viral contig files
            output_dir: Output directory (optional)
            
        Returns:
            Dictionary containing viral-specific QUAST results
        """
        if not viral_files:
            logger.warning("No viral files provided for viral QUAST")
            return {"status": "skipped", "reason": "No viral files provided"}
        
        if output_dir is None:
            output_dir = self.temp_dir / "viral_quast"
        
        output_dir = Path(output_dir)
        output_dir.mkdir(parents=True, exist_ok=True)
        
        # Filter existing files
        existing_files = [f for f in viral_files if os.path.exists(f)]
        if not existing_files:
            logger.warning("No existing viral files found for viral QUAST")
            return {"status": "skipped", "reason": "No existing viral files"}
        
        logger.info(f"Running viral QUAST on {len(existing_files)} viral files")
        ...
        cmd = [
            ...
            *existing_files,
            "-o", str(output_dir),
            "--threads", str(self.threads),
            "--min-contig", "100",  # Lower minimum for small viral contigs
            "--no-snps",  # Not relevant for viral contigs
            "--no-gc"     # GC content less relevant for viruses
        ]
        
        result = subprocess.run(cmd, capture_output=True, text=True)
        if result.returncode != 0:
            logger.warning(f"Viral QUAST failed: {result.stderr}")
            return {"status": "failed", "error": result.stderr}
        
        # Parse viral QUAST results
        results = self._parse_quast_results(output_dir)
        results["status"] = "completed"
        results["evaluation_type"] = "viral_specific"
        results["viral_files"] = existing_files
        
        logger.info("Viral QUAST completed successfully")
        return results
    
    def run_hybrid_quast(
        self, 
        assembly_dir: Path,
        viral_files: List[str],
        output_dir: Optional[str] = None
        ...
        """
        Run both assembly-level and viral-specific QUAST evaluations.
        ...
        Returns:
            Dictionary containing both assembly and viral QUAST results
        """
        if output_dir is None:
            output_dir = self.temp_dir / "hybrid_quast"
        ...
        # 1. Full assembly evaluation
        assembly_quast = self.run_comprehensive_quast(assembly_dir, output_dir / "assembly_quast")
        
        # 2. Viral contig evaluation
        viral_quast = self.run_viral_quast(viral_files, output_dir / "viral_quast")
        
        # Combine results
        hybrid_results = {
            "status": "completed",
            "evaluation_type": "hybrid",
            "assembly_quality": assembly_quast,
            "viral_quality": viral_quast,
            "output_directory": str(output_dir)
        }
        
        logger.info("Hybrid QUAST evaluation completed successfully")
    ...
    def _parse_quast_results(self, output_dir: Path) -> Dict[str, Union[str, float, int]]:
        """Parse QUAST results from output directory."""
        results = {"status": "completed"}
        
        # Look for QUAST report file
        ...
        if report_file.exists():
            try:
                ...
                # Parse QUAST report (simplified)
                lines = content.strip().split('\n')
                for line in lines:
                    if 'Total length' in line and len(line.split()) > 1:
                        try:
                            results["total_length"] = int(line.split()[1].replace(',', ''))
                        except (ValueError, IndexError):
                            pass
                    elif 'N50' in line and len(line.split()) > 1:
                        try:
                            results["n50"] = int(line.split()[1].replace(',', ''))
                        except (ValueError, IndexError):
                            pass
                    elif 'Largest contig' in line and len(line.split()) > 1:
                        try:
                            results["largest_contig"] = int(line.split()[1].replace(',', ''))
                        except (ValueError, IndexError):
                            pass
                    elif 'GC (%)' in line and len(line.split()) > 1:
                        try:
                            results["gc_percentage"] = float(line.split()[1])
                        except (ValueError, IndexError):
                            pass
                
            except Exception as e:
                logger.warning(f"Failed to parse QUAST results: {e}")
    ...
    def generate_quality_report(
        ...
        for genome_file in genome_files:
            ...
            quast_results = self.run_quast(genome_file)
